...
bool Image::loadUncompressed8BitTGA(const char * filename)
{
  ...
  //load the palette
  if (fread(palette, 256*3, 1, file) != 1)
  {
    Logger::writeErrorLog("Error reading data for palette");
  }
  ...
  //load indices
  if (fread(indices, 1, width*height, file) != (width*height))
  {
    Logger::writeErrorLog("Error reading data for indices");
  }
